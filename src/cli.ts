...
program.command("validate [file]").description("Validate one Markdown file or all project documents").action(async (file: string | undefined) => { const result = file ? [await validateDocument(file)] : await validateAllDocuments(); for (const item of result) { console.log(`${item.valid ? "valid" : "invalid"} ${item.path}`); for (const issue of item.issues) console.log(`  ${issue.severity}: ${issue.message}`); } if (result.some((item) => !item.valid)) process.exitCode = 1; });
