...
import { exists, findProjectRoot, isDirectory, resolveHarnessDirectory } from "./paths.js";
...
export async function inspectProject(from = process.cwd(), options: { persist?: boolean } = {}): Promise<Inspection> {
  ...
  if (options.persist !== false && await isDirectory(join(root, harnessDirectory))) {
    await writeFile(join(root, harnessDirectory, "inspect.json"), JSON.stringify(inspection, null, 2) + "\n");
