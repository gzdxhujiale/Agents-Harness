...
describe("initializeHarness", () => {
  it("creates only applicable templates with explicit initialization markers", async () => { const root = await mkdtemp(join(tmpdir(), "aiharness-")); directories.push(root); const result = await initializeHarness(root); expect(result.files.filter((file) => file.status === "created")).not.toHaveLength(0); expect(await readFile(join(root, "AGENTS.md"), "utf8")).toMatch(/^# /); await expect(access(join(root, ".aiharness", "changes"))).resolves.toBeUndefined(); await expect(access(join(root, "docs", "BACKEND.md"))).rejects.toThrow(); await expect(access(join(root, ".agents", "skills", "write-backend", "SKILL.md"))).rejects.toThrow(); await expect(validateDocument(join(root, "ARCHITECTURE.md"))).resolves.toMatchObject({ valid: false, issues: expect.arrayContaining([expect.objectContaining({ code: "PLACEHOLDER_REMAINING" })]) }); await expect(access(join(root, "openspec"))).rejects.toThrow(); });
